...
class Check_conn():
	...
	def test_conn(self):
		try:
			self._response = os.system("ping -c 1 " + self._test_hostname + " > /dev/null")

			if self._response == 0:
				print("The host [", self._test_hostname, "] answer. Connection OK.")
			else:
				print("[ERROR] The host [", self._test_hostname, "] doesn't answer. Check your connection.")
				sys.exit()
		except OSError:
