...
class Downloader(threading.Thread):
	...
	def download_file(self, url):
		...
		try:
			...
			#report success
			self._ok_dd_log = "\n[OK] [time: "+str((datetime.now()).strftime('%Y/%m/%d %H:%M:%S'))+"] url: "+url+" downloaded."
			self._down_log.logger(self._ok_dd_log)

		except (ValueError, OSError):
			#report errors
			print("[ERROR]", url, "Unable to resolve URL and/or write file on disk.")
			self._fail_dd_log = "\n[ERROR] [time: "+str((datetime.now()).strftime('%Y/%m/%d %H:%M:%S'))+"] Download failed for "+url
...
def main(urls, n_urls):
	...
	try:
		...
		print("\n[NOTICE] Session completed in: %s seconds" % (time.time() - start_time))
	
	except KeyboardInterrupt:
		print("NOTICE] Script stopped during execution. (Ctrl+C)")
		exec_log = "\n[ERROR] [time: "+str((datetime.now()).strftime('%Y/%m/%d %H:%M:%S'))+"] Script interrupted (Ctrl+C)!"
...
if __name__ == "__main__":
	...
	try:
		...
		print("[NOTICE] Script stopped during execution and/or crashed. Try restarting it.")
