	...
	def setup(self):
		...
		try:
			with open('config.ini') as self._cfg:
				...
				self._a_filters = (self._config.get("Filters", "filters")).split(', ')
		...
		try:
			...
			if self._a_filters[0]=='' and len(self._a_filters)==1 :
				...
				self._filtered_urls = self._filtered_urls.apply_filters(self._a_filters, self._sources)

			#for logging...
			self._conf_log = "\n[CONFIG] [time: "+str((datetime.now()).strftime('%Y/%m/%d %H:%M:%S'))+"] Setup complete. [OK]"
			self._setup_log.logger(self._conf_log)

		except:
			print("[ERROR] Unable to complete script setup.")
			self._fail_conf_log = "\n[ERROR] [time: "+str((datetime.now()).strftime('%Y/%m/%d %H:%M:%S'))+"] Setup Failed!!!"
